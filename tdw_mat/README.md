...
> This folder is a modified fork of TDW-MAT / CoELA. For the paper code, prompt templates, and reproduction instructions, see the repository root `README.md`. The original CoELA example scripts under `scripts/` may not match the current argument parser; use `scripts/run_experiment.py` at the repository root for the paper settings.
