...
class TransportChallenge(AssetCachedController):
    ...
    """
    TDW_VERSION: str = "1.13.0.0"  # 1.11.18 # 1.13.0.0
    """:class_var
    ...
    def __init__(self, port: int = 1071, check_version: bool = True, launch_build: bool = True, screen_width: int = 256,
                 screen_height: int = 256, image_frequency: ImageFrequency = ImageFrequency.always, png: bool = True, asset_cache_dir = "transport_challenge_asset_bundles",
    ...
    def start_floorplan_trial(self, scene: str, layout: int, num_containers: int, num_target_objects: int,
                              ...
                              lighting: bool = True, random_seed: int = None, data_prefix = 'dataset/dataset_train', same_room_init: bool = False) -> None:
        """
        ...
        self._start_trial_new(replicants=replicants, task_type = task, random_seed=random_seed, same_room_init=same_room_init)
    ...
    def _start_trial_new(self, replicants: Union[int, List[Union[int, np.ndarray, Dict[str, float]]]] = 2, task_type = 'food', random_seed: int = None, same_room_init: bool = False) -> None:
        """
        Start a trial in a floorplan scene.
        food or stuff
        """
        ...
        platform_dir = {"Windows": "windows", "Linux": "linux", "Darwin": "osx"}.get(system(), "linux")
        for cmd in scene:
            url = cmd.get("url")
            if isinstance(url, str):
                if url.startswith("https://tdw-public.s3.amazonaws.com/models/"):
                    rest = url[len("https://tdw-public.s3.amazonaws.com/models/"):]
                    parts = rest.split("/", 1)
                    if len(parts) == 2:
                        cmd["url"] = f"https://tdw-public.s3.amazonaws.com/models/{platform_dir}/{parts[1]}"
                elif url.startswith("https://tdw-public.s3.amazonaws.com/hdri_skyboxes/"):
                    rest = url[len("https://tdw-public.s3.amazonaws.com/hdri_skyboxes/"):]
                    parts = rest.split("/", 1)
                    if len(parts) == 2:
                        cmd["url"] = f"https://tdw-public.s3.amazonaws.com/hdri_skyboxes/{platform_dir}/{parts[1]}"
                elif url.startswith("https://tdw-public.s3.amazonaws.com/replicants/"):
                    rest = url[len("https://tdw-public.s3.amazonaws.com/replicants/"):]
                    parts = rest.split("/", 1)
                    if len(parts) == 2:
                        cmd["url"] = f"https://tdw-public.s3.amazonaws.com/replicants/{platform_dir}/{parts[1]}"
        ...
        if isinstance(replicants, int):
            ...
            if same_room_init and len(room_indices) > 0:
                anchor_position = None
                if "0" in count_and_position:
                    anchor_position = count_and_position["0"]
                if anchor_position is not None:
                    replicant_positions = self._sample_same_room_positions_from_anchor(
                        anchor_position=anchor_position,
                        rooms=rooms,
                        num_replicants=replicants,
                    )
                else:
                    shared_room = room_indices[0]
                    positions = sorted(rooms[shared_room], key=lambda p: (p["x"], p["z"]))
                    replicant_positions = positions[:replicants]
            else:
                room_index = 0
        ...
        # Reuse dataset-fixed spawn points only when they satisfy the current room constraint.
        # If metadata positions happen to violate the requested constraint, keep the freshly sampled positions instead.
        metadata_positions = []
        metadata_complete = len(replicant_positions) > 0 and all(str(i) in count_and_position for i in range(len(replicant_positions)))
        if metadata_complete:
            metadata_positions = [count_and_position[str(i)] for i in range(len(replicant_positions))]
            metadata_room_indices = [self._get_room_index_of_position(position) for position in metadata_positions]
            metadata_known_room_indices = [room_index for room_index in metadata_room_indices if room_index is not None]
            metadata_same_room = len(metadata_known_room_indices) > 0 and len(set(metadata_known_room_indices)) == 1

            metadata_matches_constraint = metadata_same_room if same_room_init else not metadata_same_room
            if metadata_matches_constraint:
                replicant_positions = metadata_positions
        ...
        reset_heads = False
        num_reset_head_frames = 0
        while not reset_heads:
            self.communicate([])
            num_reset_head_frames += 1
            if num_reset_head_frames >= 2000:
                self._print_build_log()
                raise RuntimeError("Timed out while waiting for replicants to finish reset_head")
            reset_heads = True
            for replicant_id in self.replicants:
    ...
    def _get_room_index_of_position(self, position: Dict[str, float]) -> Optional[int]:
        x = float(position["x"])
        z = float(position["z"])
        for i, region in enumerate(self._scene_bounds.regions):
    ...
    def _sample_same_room_positions_from_anchor(self, anchor_position: Dict[str, float], rooms: Dict[int, List[Dict[str, float]]], num_replicants: int) -> List[Dict[str, float]]:
        anchor_room = self._get_room_index_of_position(anchor_position)
        if anchor_room is None or anchor_room not in rooms or len(rooms[anchor_room]) == 0:
            fallback_room = sorted(rooms.keys())[0]
            candidates = rooms[fallback_room]
            anchor = np.array([candidates[0]["x"], 0.0, candidates[0]["z"]], dtype=np.float32)
        else:
            candidates = rooms[anchor_room]
            anchor = np.array([anchor_position["x"], 0.0, anchor_position["z"]], dtype=np.float32)

        def distance_to_anchor(candidate: Dict[str, float]) -> float:
            dx = float(candidate["x"]) - float(anchor[0])
            dz = float(candidate["z"]) - float(anchor[2])
            return dx * dx + dz * dz

        sorted_candidates = sorted(candidates, key=distance_to_anchor)
        selected: List[Dict[str, float]] = []
        min_separation = 0.5

        for candidate in sorted_candidates:
            if len(selected) >= num_replicants:
                break
            if len(selected) == 0:
                selected.append(candidate)
                continue

            candidate_vec = np.array([candidate["x"], 0.0, candidate["z"]], dtype=np.float32)
            valid = True
            for chosen in selected:
                chosen_vec = np.array([chosen["x"], 0.0, chosen["z"]], dtype=np.float32)
                if np.linalg.norm(candidate_vec - chosen_vec) < min_separation:
                    valid = False
                    break
            if valid:
                selected.append(candidate)
        ...
        return [dict(p) for p in selected[:num_replicants]]
